...
(async () => {
  console.log('🎬 Recording IMPROVED Image-to-3D (4-phase, no dialog)...');
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .selected-display {
      ...
      box-shadow: 0 0 40px rgba(59, 145, 141, 0.4),
                  inset 0 0 60px rgba(59, 145, 141, 0.1);
    }
    ...
    .generate-btn {
      ...
      box-shadow: 0 4px 20px rgba(59, 145, 141, 0.4);
      cursor: pointer;
...
<body>
  ...
  if (videoFile) {
    ...
    const newPath = path.join(__dirname, 'image-to-3d-v2-improved.mp4');
    fs.renameSync(oldPath, newPath);
    fs.unlinkSync(path.join(__dirname, 'record-temp.html'));
    if (fs.existsSync(videoDir)) fs.rmdirSync(videoDir);
    
    console.log('✅ Video saved: image-to-3d-v2-improved.mp4');
  }
})();
