...
(async () => {
  console.log('🎬 Recording Temple Image-to-3D (improved 4-phase)...');
  ...
  const imagePath = path.join(__dirname, 'user-image-temple.jpg');
...
<body>
  ...
  if (videoFile) {
    ...
    const newPath = path.join(__dirname, 'image-to-3d-temple.mp4');
    fs.renameSync(oldPath, newPath);
    fs.unlinkSync(path.join(__dirname, 'record-temp.html'));
    if (fs.existsSync(videoDir)) fs.rmdirSync(videoDir);
    
    console.log('✅ Video saved: image-to-3d-temple.mp4');
  }
})();
